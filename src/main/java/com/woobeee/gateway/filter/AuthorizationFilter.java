...
import com.woobeee.gateway.util.ErrorResponse;
import com.woobeee.gateway.util.Response;
import com.woobeee.gateway.util.TokenDetails;
...
public class AuthorizationFilter extends AbstractGatewayFilterFactory<AuthorizationFilter.Config> {
	...
	private Mono<Void> onError(ServerWebExchange exchange, String err, HttpStatus httpStatus) {
		...
		Response<ErrorResponse> resp = new Response<>(
			new Response.Header(false, HttpStatus.UNAUTHORIZED.value()),
			new Response.Body<>(ErrorResponse.builder()
				.title(err)
				.status(httpStatus.value())
				.timestamp(LocalDateTime.parse(String.valueOf(System.currentTimeMillis())))
				.build()));
